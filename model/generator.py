...
class DynamicSkipLayer(BaseNetwork):
    ...
    def forward(self, fdec, fenc, s):
        ...
        state = F.leaky_relu(self.trans(self.up(s)), 2e-1)                   # h^ = sigma(Wh * up(h))
        reset_gate = self.sigmoid(self.reset(torch.cat((state,fenc),dim=1))) # r = sigma(Wr * [h^, fE])
        new_state = reset_gate * state                                       # h = rh^
        mask = self.mask(torch.cat((state,fenc),dim=1))                      # m = sigma(Wm * [h^, fE]) with sigma=None
        mask = (F.relu(mask)).tanh()                                         # apply relu + tanh to set most of the elements to zeros
        new_fenc = self.update(torch.cat((new_state,fenc),dim=1))            # fE^ = sigma(WE * [h, fE]) with sigma=None
        output = (1-mask) * fdec + mask * new_fenc                            # f = (1-m) * fG + m * fE^
